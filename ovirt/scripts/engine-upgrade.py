...
MSG_INFO_UPDATE_ENGINE_SYSCONFIG = "Updating service configuration"
...
MSG_OVIRT_ENGINE_RECREATED = "\nINFO: The configuration file %s was backed up and a new\n\
file with more updated settings was created instead." % basedefs.FILE_OVIRT_HTTPD_CONF
...
def restartHttpd(service=basedefs.HTTPD_SERVICE_NAME):
    ...
    cmd = [
        basedefs.EXEC_SERVICE, service, "restart",
    ]
    output, rc = utils. execCmd(cmdList=cmd, failOnError=True, msg="Can't restart HTTPD service")
...
def editPgpass(pgpass=""):
    ...
    cmd = [
        "/bin/sed", "-i", "-e", "s/:engine:engine/:*:engine/", pgpass
    ]
    output, rc = utils.execCmd(cmdList=cmd, failOnError=False)
