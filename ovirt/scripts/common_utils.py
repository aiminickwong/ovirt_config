...
def updateEngineSysconfig():
    '''
    Configure ovirt-engine post upgrade
    '''
    # Get DB Pass
    dbPassword = getDbPassword(basedefs.DB_USER)

    # Set DB Url
    dbUrl = "jdbc:postgresql://%s:%s/%s" % (
        getDbHostName(),
        getDbPort(),
        basedefs.DB_NAME,
    )

    # Update DB url with secured connection
    if os.path.exists(basedefs.OLD_ENGINE_SERVICE_XML):
        xmlObj = XMLConfigFileHandler(basedefs.OLD_ENGINE_SERVICE_XML)
        xmlObj.open()
        xmlObj.registerNs(
            'datasource',
            xmlObj.getNs('urn:jboss:domain:datasources'),
        )
        xpath = xmlObj.xpathEval(
            "//datasource:subsystem/datasource:datasources/"
            "datasource:datasource[@jndi-name='java:/ENGINEDataSource']/"
            "connnection-property[@name='ssl']"
        )

        if xpath and xpath[0]:
            # Means ssl mode was set
            dbUrl = dbUrl + "?ssl=true&sslfactory=org.postgresql.ssl.NonValidatingFactory"

    # Get current configuration

    webConfigFile = basedefs.FILE_WEB_CONF

    if not os.path.exists(basedefs.FILE_WEB_CONF):
        if not os.path.exists(basedefs.FILE_WEB_CONF+".rpmsave"):
            raise Exception("Can't find web config file")
        else:
            webConfigFile = basedefs.FILE_WEB_CONF+".rpmsave"

    fqdn, httpPort, httpsPort = getHostParams(webConfigFile)

    # Get encrypted password
    masked_value_set = [dbPassword]
    encryptedPassword = encryptEngineDBPass(password=dbPassword,
                                            maskList=masked_value_set)

    proxyEnabled = False
    # If proxy already enabled, update the value
    # of the proxyEnabled to True
    if os.path.exists(basedefs.FILE_OVIRT_HTTPD_CONF):
        ...
        shutil.move(basedefs.FILE_OVIRT_HTTPD_CONF, backup_file)

    # Regenerate the ovirt-engine.conf for httpd from the template
    redirectUrl()
    ...
    configEncryptedPass(encryptedPassword)

    logging.debug("ovirt-engine configuration was successfully updated")
