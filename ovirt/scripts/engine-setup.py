...
def generateOrgName2():
    fqdn = socket.getfqdn().split('.',2)
    ...
    sequences_conf = [
                      ...
                      { 'description'     : 'Edit Configuration',
                       ...
                       { 'description'     : 'License Configuration',
                        'condition'       : [],
                        'condition_match' : [],
                        'steps'           : [ { 'title'     : output_messages.INFO_UPD_LICENSE_CONF,
                                                'functions' : [_editLicenseConf] } ]
                       },
                      ...
                      { 'description'     : 'Handling httpd',
                        ...
                        'steps'           : [ { 'title'     : output_messages.INFO_CONFIG_HTTPD,
                                                'functions' : [_configureSelinuxBoolean, _backupOldHttpdConfig, _configureHttpdSslKeys, _configureHttpdPort, _configureHttpdSslPort, utils.redirectUrl, _startHttpd]}]
                       },
                     ]
...
def initConfig():
    ...
    NEED_GENERATE    - (True/False) is this param need to be generated?
         ...
         "ALL_PARAMS" : [
            ...
            {   "CMD_OPTION"      :"auth-pass",
             ...
             {  "CMD_OPTION"      :"org-name",
                ...
                "DEFAULT_VALUE"   :generateOrgName2(),
                "MASK_INPUT"      : False,
                "LOOSE_VALIDATION": False,
                "CONF_NAME"       : "ORG_NAME",
                "USE_DEFAULT"     : False,
                ...
                "NEED_GENERATE"   : False,
                "CONDITION"       : False},
			 {  "CMD_OPTION"       :"org-license",
                "USAGE"           :output_messages.INFO_CONF_PARAMS_ORG_LICENSE_USAGE,
                "PROMPT"          :output_messages.INFO_CONF_PARAMS_ORG_LICENSE_PROMPT,
                "OPTION_LIST"     :[],
                "VALIDATION_FUNC" :validate.validateOrgName,
                "DEFAULT_VALUE"   :"",
                "MASK_INPUT"      : False,
                "LOOSE_VALIDATION": False,
                "CONF_NAME"       : "ORG_LICENSE",
                "USE_DEFAULT"     : False,
                "NEED_CONFIRM"    : False,
                "NEED_GENERATE"   : True,
                "CONDITION"       : False},
...
def input_param(param):
    ...
    """
    if(param.getKey("NEED_GENERATE")):
        #name = get_hardwareid.get_hardwareid()        
        while True:
            _getInputFromUser(param)
            license = controller.CONF["ORG_LICENSE"]
        #    name = controller.CONF["ORG_NAME"]
            try:
		licenseDe = des.strdesde(license,desKey)
		licenseName = licenseDe[0:4]
        	name = get_hardwareid.get_hardwareid(license)        
		#if (licenseName == md5.strmd5(name)[0:4]):
		if ( name == True ):
		    logging.debug("Param generation passed, value for the questions is valid")
                    vmAmount = licenseDe[4:8]
                    deadLine = licenseDe[8:16]
                    break
                else:
                    print output_messages.INFO_INVAID_LICENSE
            except:
                print output_messages.INFO_INVAID_LICENSE
            
    # We need to check if a param needs confirmation, (i.e. ask user twice)
    # Do not validate if it was given from the command line
    elif (param.getKey("NEED_CONFIRM") and not commandLineValues.has_key(param.getKey("CONF_NAME"))):
...
def _addLicensetoDB(license,name,vmAmount,deadLine):
    logging.debug("Adding license info into DB")
    stamp = int(time.time())
    sqlQuery = "select inst_license_info ('%s', '%s', %s, %s, %s)" % (license, name, vmAmount, deadLine,stamp)
    utils.execRemoteSqlCommand(getDbUser(), getDbHostName(), getDbPort(), basedefs.DB_NAME, sqlQuery, True, output_messages.ERR_FAILED_INSERT_LICENSE%(basedefs.DB_NAME))
